...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        local timeToPredict = getgenv().TriggerBot.Settings.Preds.PredictionMultiplier or 0.08
        local predictedPosition = primaryPart.Position + (velocity * timeToPredict)

        if getgenv().TriggerBot.Settings.Preds.EnableLegitMode and character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
...
local function findTargetPart(character)
    local partList = getgenv().TriggerBot.Settings.Preds.EnableLegitMode and AllBodyParts or getgenv().TriggerBot.Settings.Parts
...
local function isIgnoringKnife()
    ...
    if currentTool then
        ...
        return getgenv().TriggerBot.Settings.Preds.Safety.IgnoreKnife and (toolName == "knife" or toolName == "katana" or toolName == "[knife]" or toolName == "[katana]")
    end
    return false
end
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                if predictedPosition and isWithinBoxFOV(predictedPosition) then
                    local closestPart = findTargetPart(TargetPlayer.Character)
                    if closestPart then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then
        local keyMode = getgenv().TriggerBot.Settings.Mode
        if input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.TriggerBot:upper()] then
            isToggled = keyMode == "toggle" and not isToggled or keyMode == "hold"
        end
    end
end)

UserInputService.InputEnded:Connect(function(input, gameProcessed)
    if not gameProcessed and getgenv().TriggerBot.Settings.Mode == "hold" and input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.TriggerBot:upper()] then
        isToggled = false
    end
end)
